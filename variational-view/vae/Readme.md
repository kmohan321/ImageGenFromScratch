...
$$
\mathcal{L}(\phi, \theta) = \underbrace{\mathbb{E}_{q_\phi(z|x)} \left[ \log p_\theta(x|z) \right]}_{\text{Term 1}} - \underbrace{D_{KL}(q_\phi(z|x) \parallel p(z))}_{\text{Term 2}}
$$
...
1.  **Term 1: Reconstruction Loss**
    ...
    * As you noted, if we assume a Gaussian decoder, this term simplifies to **Mean Squared Error (MSE)**. If we assume a Bernoulli decoder, it simplifies to **Binary Cross-Entropy (BCE)**.

2.  **Term 2: Regularization Loss (KL Divergence)**
