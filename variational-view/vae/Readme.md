...
```math
D_{\text{KL}}(q_{\phi}(z|x) \parallel p_{\theta}(z|x))
```

By definition, a KL divergence is always non-negative: $D_{\text{KL}}(\dots) \ge 0$.

Now, let's expand the definition of KL divergence:

```math
D_{\text{KL}}(q_{\phi}(z|x) \parallel p_{\theta}(z|x)) = \mathbb{E}_{q_{\phi}(z|x)} \left[ \log \left( \frac{q_{\phi}(z|x)}{p_{\theta}(z|x)} \right) \right]
```
...
```math
D_{\text{KL}}(q_{\phi}(z|x) \parallel p_{\theta}(z|x)) = \mathbb{E}_{q_{\phi}(z|x)} \left[ \log q_{\phi}(z|x) - \log p_{\theta}(x,z) \right] + \log p_{\theta}(x)
```

Let's rearrange this to solve for our original (intractable) goal, $\log p_{\theta}(x)$:

```math
\log p_{\theta}(x) = D_{\text{KL}}(q_{\phi}(z|x) \parallel p_{\theta}(z|x)) - \mathbb{E}_{q_{\phi}(z|x)} \left[ \log q_{\phi}(z|x) - \log p_{\theta}(x,z) \right]
```

Flip the sign in the expectation term by inverting the fraction:

```math
\log p_{\theta}(x) = D_{\text{KL}}(q_{\phi}(z|x) \parallel p_{\theta}(z|x)) + \mathbb{E}_{q_{\phi}(z|x)} \left[ \log \left( \frac{p_{\theta}(x,z)}{q_{\phi}(z|x)} \right) \right]
```
...
```math
\log p_{\theta}(x) = \underbrace{D_{\text{KL}}(q_{\phi}(z|x) \parallel p_{\theta}(z|x))}_{\text{The "KL Gap"} \ge 0} + \underbrace{\mathcal{L}(\phi, \theta)}_{\text{The ELBO}}
```

Where $\mathcal{L}(\phi, \theta) = \mathbb{E}_{q_{\phi}(z|x)} \left[ \log \left( \frac{p_{\theta}(x,z)}{q_{\phi}(z|x)} \right) \right]$.

This tells us:
1.  **It's a "Lower Bound"**: Since the $D_{\text{KL}}$ term is always $\ge 0$, we know that $\log p_{\theta}(x) \ge \mathcal{L}(\phi, \theta)$. This means $\mathcal{L}$ is a **Lower Bound** on the (log) Evidence.
...
3.  **A Dual Goal**: Maximizing $\mathcal{L}$ is also *equivalent* to minimizing the $D_{\text{KL}}$ "gap," which forces our approximation $q_{\phi}(z|x)$ to become a better match for the true posterior $p_{\theta}(z|x)$.
...
The second term is just the definition of the KL divergence: $\mathbb{E}_q \left[ \log \left( \frac{q}{p} \right) \right] = D_{\text{KL}}(q \parallel p)$.

```math
\mathcal{L}(\phi, \theta) = \underbrace{\mathbb{E}_{q_{\phi}(z|x)} \left[ \log p_{\theta}(x|z) \right]}_{\text{Term 1: Reconstruction}} - \underbrace{D_{\text{KL}}(q_{\phi}(z|x) \parallel p(z))}_{\text{Term 2: Regularization}}
```
...
```math
\text{Loss} = - \mathbb{E}_{q_{\phi}(z|x)} \left[ \log p_{\theta}(x|z) \right] + D_{\text{KL}}(q_{\phi}(z|x) \parallel p(z))
```
...
```math
D_{\text{KL}}(q_{\phi}(z|x) \parallel p(z)) = -\frac{1}{2} \sum_{j=1}^{J} \left( 1 + \log(\sigma_j^2) - \mu_j^2 - \sigma_j^2 \right)
```
